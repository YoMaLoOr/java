        ...
        <div id="contPrincipal">
            <c:choose>
                <c:when test="${resu eq 'ok'}">
                    <p>El usuario <span>${nombreUsuario}</span> se ha registrado correctamente.</p>
                </c:when>
                <c:when test="${resu eq 'existe'}">
                    <p>El usuario <span>${nombreUsuario}</span> ya existe.</p>
                    <a href="${home}/registro_usuarios"><button>Registro Usuarios</button></a>
                </c:when>
                <c:when test="${resu eq 'error'}">
                    <p>Error al crear el usuario <span>${nombreUsuario}</span>, intentelo nuevamente.</p>
                    <a href="${home}/registro_usuarios"><button>Registro Usuarios</button></a>
                </c:when>
            </c:choose>
